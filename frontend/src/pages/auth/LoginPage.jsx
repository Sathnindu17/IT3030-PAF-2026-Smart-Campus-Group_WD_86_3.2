...
export default function LoginPage() {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      setError('');
      try {
        // Send access token to our backend which will verify it with Google
  ...
  return (
    <div className="auth-container">
      <div className="auth-card">
        <h1>Welcome Back</h1>
        <p className="subtitle">Sign in to Smart Campus Hub</p>

        {error && <div className="alert alert-error">{error}</div>}

        <button onClick={() => googleLogin()} className="google-btn" type="button" disabled={googleLoading}>
